...
Timekeepers requires Julia 1.10 or newer.
...
!!! tip
    As a general Julia best practice, avoid installing packages into your
    default (global) environment. A dedicated per-project environment keeps
    dependencies isolated and reproducible, and avoids slow, unexpected version
    changes across unrelated packages you already have installed.
...
The `julia --project=.` prefix used throughout these docs activates that cloned
environment.
