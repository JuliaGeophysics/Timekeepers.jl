...
The **View** menu in [TKApp](tkapp.md) adds two diagnostic panels next to the
time series. Both are there to answer the question you actually have while
...
## `Time | Spectrogram` — STFT

A short-time Fourier transform of each channel: time on x, frequency on y
(logarithmic), and log₁₀ power as colour.

Any STFT window that touches even one masked sample is written as `NaN` and
renders as a blank column, so masked intervals appear as vertical gaps. That
makes the spectrogram a useful check on your own masking: if a noise band
extends past the edge of a blank column, the cut was too narrow.

## How the transform length is chosen

`nfft` is derived from the visible window rather than fixed, so the panels stay
...
!!! note "Internal API"
    ...
    `Timekeepers._welch_psd_segments`, `Timekeepers._stft_psd` and
    `Timekeepers.SpectralWorkspace`) are internal and not covered by semantic
    versioning. For spectral analysis in your own code, take clean segments out
    of Timekeepers with [`good_segments`](@ref) and use a dedicated package such
    as [DSP.jl](https://github.com/JuliaDSP/DSP.jl):
